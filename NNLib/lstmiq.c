...
// Updated weights for LSTM with 1 node in C format:

real_T Wi[2] = {0.3791226, -0.0166609};
real_T Wf[2] = {-0.34560528, -0.13878638};
real_T Wo[2] = {-0.33595467, 0.07413457};
real_T Wc[2] = {-0.3162397, -0.3285082};

double Ui = -0.13903658;
double Uf = -0.2975196;
double Uo = 0.07816567;
double Uc = 0.17829163;

double bi = 0.19482672;
double bf = 1.3657101;
double bo = 0.93662995;
double bc = -0.00822883;
...
if (fabs(LSTMtestfive_Y.y_hat - prev )> limit){

		if (LSTMtestfive_Y.y_hat - prev > 0){
			LSTMtestfive_Y.y_hat = prev + limit;
		}
		else{
			LSTMtestfive_Y.y_hat = prev - limit;
			}
