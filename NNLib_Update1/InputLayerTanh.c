...
for (k = 0; k<Y; k++){
	indx = 4*k;
	a1[k] = tanh(W[0+indx] * X[0]+ W[1+indx]*X[1] + W[2				+indx]*X[2] +W[3+indx]*X[3] + b[k]); 
	da[k] = 1 - a1[k]*a1[k];
...
static void Backprop(double Loss[6], double da1[6], int N, int Y, double alpha){
...
Backprop(loss, da11, N, Y, Learning_rate);
